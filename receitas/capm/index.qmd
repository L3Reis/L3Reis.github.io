---
title: "Estimação do CAPM (Market Model)"
description: "Cálculo do Risco Sistemático (Beta) via Covariância e OLS."
date: "2026-08-14"
categories: [Finanças Quantitativas, Asset Pricing, Python]
---

## Premissas do Modelo
* Trata-se de um CAPM simplificado (Market Model), assumindo $R_f = 0$;
* Os preços são ajustados por proventos (`auto_adjust=True`);
* Os retornos são discretos e mensais;
* O portfólio possui pesos iguais (Equal-Weighted);
* Há rebalanceamento mensal implícito na alocação;
* O $\beta$ é calculado por covariância e confirmado por regressão linear (OLS).
